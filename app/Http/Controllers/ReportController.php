    ...
    public function averageCost($product_id , request $request)
    {
     ...
     public function weeklyChart(){
        ...
        foreach ($weeks as $week) {
            ...
            $weekly_ads = Ad::
            whereDate('start_date','>=',SupportCarbon::make($week['from'])->toDateString())
            ->whereDate('end_date','<=', SupportCarbon::make($week['to'])->toDateString())
            ->get();
...
//        );
        ...
        //get all running ads from facebook graph api
        $ads = $getRunningAdsAction->invoke();
